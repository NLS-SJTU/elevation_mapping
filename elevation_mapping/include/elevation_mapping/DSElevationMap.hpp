...
#include "sensor_processors/LaserSensorProcessorSimple.hpp"
...
class DSElevationMap
{
 public:
   ...
   * Constructor.
   */
  DSElevationMap(ros::NodeHandle nodeHandle);

  /*!
   * Destructor.
   */
  virtual ~DSElevationMap();
  ...
  bool ds_add(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr pointCloud, Eigen::VectorXf& pointCloudVariances,
           const ros::Time& timeStamp, const Eigen::Affine3d& transformationSensorToMap);

  // poindcloud: in sensor frame
  bool ds_newadd(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr pointCloud,
  ...
  bool ds_update(const grid_map::Matrix& varianceUpdate,
              const grid_map::Matrix& horizontalVarianceUpdateX,
              const grid_map::Matrix& horizontalVarianceUpdateY,
              const grid_map::Matrix& horizontalVarianceUpdateXY, const ros::Time& time);
  ...
  /*!
   * fuse present map to static and dynamic map
   */
  bool fusePreToStaDyn();
  ...
  /*!
   * Publishes all the (latest) elevation map
   */
  void publishAllElevationMap();

  /*!
   * Publishes the (latest) static elevation map
   */
  bool publishStaticElevationMap();

  /*!
   * Publishes the (latest) dynamic elevation map
   */
  bool publishDynamicElevationMap();

  /*!
   * Publishes the (latest) present elevation map
   */
  bool publishPresentElevationMap();

  /*!
   * Publishes the (latest) final elevation map
   */
  bool publishFinalElevationMap();
...
//  grid_map::GridMap& getFusedGridMap();
  /*!
   * Gets a reference to the static map.
   * @return the static grid map.
   */
  grid_map::GridMap& getStaticGridMap();

  grid_map::GridMap& getDynamicGridMap();

  grid_map::GridMap& getPresentGridMap();

  grid_map::GridMap& getFinalGridMap();
...
//  bool hasFusedMapSubscribers() const;
 ...
 private:
  ...
  //! static elevation map as grid map.
  grid_map::GridMap staticMap_;

  //! dynamic elevation map as grid map.
  grid_map::GridMap dynamicMap_;

  //! present elevation map as grid map(with only present observation).
  grid_map::GridMap preMap_;

  //! final elevation map as grid map.
  grid_map::GridMap finalMap_;
  ...
  //! ROS publishers.
  ros::Publisher elevationMapStaticPublisher_;
  ros::Publisher elevationMapDynamicPublisher_;
  ros::Publisher elevationMapFinalPublisher_;
  ros::Publisher elevationMapPresentPublisher_;
  ...
  double dtos_threshold;
  int length_grid_num;
  double obj_speed_factor;

  double error_measure_threshold;
  double max_rate_error_grids;
  double dynamic_variance_threshold;

  LaserSensorProcessorSimple::Ptr sensorProcessor_;
